...
void ALobbyGameMode::PostLogin(APlayerController* NewPlayer) {
	...
	if (HasAuthority())
	{
		...
		if (NewPlayer) {
			...
			if (this->CandyChaosLobbyGameInstance != nullptr) {

				MaxPlayers = this->CandyChaosLobbyGameInstance->MaxPlayers;

				FConfigurationMaps* DefaultMap = this->CandyChaosLobbyGameInstance->ConfigurationMaps.Find(TEXT("Cementery"));

				UTexture2D* mapImage = Cast<UTexture2D>(StaticLoadObject(UTexture2D::StaticClass(), NULL, *DefaultMap->ImagePath));

				LobbyPlayerController->Client_SetupLobbyMenu(this->CandyChaosLobbyGameInstance->ServerName);
...
void ALobbyGameMode::Server_ShouldHideLoadingScreen_Implementation(ALobbyPlayerController* LobbyPlayerController)
{
    if (this->AllPlayerControllers.Contains(LobbyPlayerController)) {
        UCandyChaosLoobyBlueprintLibrary::HideLoadingScreen(this);
    }
}
...
void ALobbyGameMode::SetPlayerInfoToTransfer()
{
	for (ALobbyPlayerController* LobbyPlayerController : this->AllPlayerControllers)
	{
		...
		this->CandyChaosLobbyGameInstance->InGamePlayersInfo.Add(InGamePlayerInfo);
	}
}
...
TArray<FPlayerKickNameIndex> ALobbyGameMode::GetPlayerKickNameIndex()
{
	...
	for (ALobbyPlayerController* PlayerController : this->AllPlayerControllers) {
		...
		ACandyChaosLobbyPlayerState* CandyChaosLobbyPlayerState = PlayerController->GetPlayerState<ACandyChaosLobbyPlayerState>();

		if (IsValid(CandyChaosLobbyPlayerState)) {

			FPlayerKickNameIndex PlayerKickNameIndex;

			PlayerKickNameIndex.Name = FText::FromString(CandyChaosLobbyPlayerState->GetPlayerName());
...
{
	FLobbyHeroeSpot* LobbyHeroeSpot = this->CandyChaosLobbyGameInstance->ConfigurationLobbyHeroeSpot.Find(LobbyPlayerController->PlayerSettings.PlayerIndex);
