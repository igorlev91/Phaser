...
#include "Common/RReplicationGraph.h"
...
{
	URReplicationGraph* ReplicationGraph = Cast<URReplicationGraph>(GetOuter());
...
void URReplicationGraph::InitConnectionGraphNodes(UNetReplicationGraphConnection* ConnectionManager)
{
...
void URReplicationGraph::RemoveClientConnection(UNetConnection* NetConnection)
{
...
void URReplicationGraph::ResetGameWorldState()
{
...
void URReplicationGraph::RouteAddNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo,
...
void URReplicationGraph::RouteRemoveNetworkActorToNodes(const FNewReplicatedActorInfo& ActorInfo)
{
	if (ActorInfo.Class->IsChildOf(AGameStateBase::StaticClass()) || ActorInfo.Class->IsChildOf(APlayerState::StaticClass()))
	{
...
void URReplicationGraph::SetTeamForPlayerController(APlayerController* PlayerController, int32 Team)
{
	if (PlayerController)
	{
...
void URReplicationGraph::HandlePendingActorsAndTeamRequests()
{
...
UTutorialConnectionManager* URReplicationGraph::GetTutorialConnectionManagerFromActor(const AActor* Actor)
{
	if (Actor)
	{
