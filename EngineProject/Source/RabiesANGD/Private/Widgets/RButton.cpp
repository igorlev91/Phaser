...
void URButton::NativePreConstruct()
{
	...
	if (RabiesButton)
	{
		...
		RabiesButton->OnClicked.AddDynamic(this, &URButton::OnClickedButton);

		FButtonStyle rabbiesStyle = RabiesButton->GetStyle();
...
void URButton::OnHoverButton()
{
	...
	{
		OnItemHovered.Broadcast(GivenItem);
	}
...
void URButton::OnClickedButton()
{
	OnToyBoxClicked.Broadcast(GivenItem);
}
