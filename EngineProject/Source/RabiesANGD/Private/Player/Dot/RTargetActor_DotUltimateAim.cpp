...
ARTargetActor_DotUltimateAim::ARTargetActor_DotUltimateAim()
{
...
void ARTargetActor_DotUltimateAim::SetTargettingRadus(float targettingRadus)
{
	TargetSphere->SetSphereRadius(targettingRadus);
}

void ARTargetActor_DotUltimateAim::SetTargettingRange(float newTargettingRange)
{
	TargettingRange = newTargettingRange;
}

void ARTargetActor_DotUltimateAim::SetOwningPlayerControler(ARPlayerController* myController)
{
	MyPlayerController = myController;
}

void ARTargetActor_DotUltimateAim::Tick(float DeltaSecond)
{
	...
	if (playerView.bBlockingHit)
	{
		...
		viewLoc += ForwardVector * 5.0f;
		viewLoc += RightVector * -20.0f;
		viewLoc += UpVector * -40.0f;
		...
		FVector Scale = FVector(0.6f, 0.6f, Length / 100.0f);

		CylinderMesh->SetRelativeScale3D(Scale);
	}
}

bool ARTargetActor_DotUltimateAim::IsConfirmTargetingAllowed()
{
	return GetPlayerView().bBlockingHit;
}

void ARTargetActor_DotUltimateAim::ConfirmTargetingAndContinue()
{
	if (MyPlayerController == nullptr)
		return;
...
FHitResult ARTargetActor_DotUltimateAim::GetPlayerView() const
{
	...
	if (MyPlayerController)
	{
		...
		FVector finalPoint = viewLoc + forwardDir * 8000.0f;
