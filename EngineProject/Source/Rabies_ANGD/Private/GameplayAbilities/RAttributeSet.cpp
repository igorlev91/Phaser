...
void URAttributeSet::PreAttributeChange(const FGameplayAttribute& Attribute, float& NewValue)
{
	...
	{
		AbilityCooldownReduction = FMath::Clamp(NewValue, 0.01f, 1.0f);
	}

	if (Attribute == GetMeleeAttackCooldownReductionAttribute())
	{
		MeleeAttackCooldownReduction = FMath::Clamp(NewValue, 0.01f, 1.0f);
	}

	if (Attribute == GetRangedAttackCooldownReductionAttribute())
	{
		RangedAttackCooldownReduction = FMath::Clamp(NewValue, 0.01f, 1.0f);
	}

	if (Attribute == GetUltimateCooldownReductionAttribute())
	{
		UltimateCooldownReduction = FMath::Clamp(NewValue, 0.01f, 1.0f);
	}
...
void URAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetRangedAttackCooldownReductionAttribute())
	{
		SetRangedAttackCooldownReduction(FMath::Clamp(GetRangedAttackCooldownReduction(), 0.01f, 1.0f));
	}
}

void URAttributeSet::OnRep_PassiveHealing(const FGameplayAttributeData& OldValue)
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(URAttributeSet, PassiveHealing, OldValue);
}

void URAttributeSet::OnRep_TestAttribute(const FGameplayAttributeData& OldValue)
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(URAttributeSet, TestAttribute, OldValue);
}
...
void URAttributeSet::GetLifetimeReplicatedProps(TArray<class FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION_NOTIFY(URAttributeSet, TestAttribute, COND_None, REPNOTIFY_Always);
	DOREPLIFETIME_CONDITION_NOTIFY(URAttributeSet, PassiveHealing, COND_None, REPNOTIFY_Always);
}
