...
void UHealthBar::SetHealth(float NewHealth, float MaxHealth)
{
	...
	AlternateProgressBar->SetPercent(Percent);
...
void UHealthBar::SetAllyView(UTexture* characterIcon)
{
	...
	ProgressBar->SetRenderOpacity(0.0f);
	ShaderBar->SetRenderOpacity(0.0f);


	//UE_LOG(LogTemp, Error, TEXT("%s Setting health bar"), *GetName());
}

void UHealthBar::SetPersonalView()
{
	AlternateProgressBar->SetRenderOpacity(0.0f);
}
