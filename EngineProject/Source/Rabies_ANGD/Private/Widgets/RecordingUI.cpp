...
void URecordingUI::LerpWarningImage(FSlateBrush slate)
{
    ...
    {
        if (ErrorImage->GetBrush().ImageSize.Y >= 120)
            return;
...
void URecordingUI::DisplayError()
{
	...
	MoveTimerHandle = GetWorld()->GetTimerManager().SetTimerForNextTick(FTimerDelegate::CreateUObject(this, &URecordingUI::LerpWarningImage, ErrorImage->GetBrush()));
}
